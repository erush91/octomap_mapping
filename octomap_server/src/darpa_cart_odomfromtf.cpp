...
int main(int argc, char** argv){
  ros::init(argc, argv, "darpa_odom_publisher");

  ros::NodeHandle node;

  ros::Publisher jackal_odom_pub = 
    node.advertise<nav_msgs::Odometry>("husky_odom", 10);

  ros::Publisher jackal_point_pub = 
    node.advertise<geometry_msgs::PointStamped>("husky_point", 1);
  ...
  while (node.ok()){
    ...
    try{
      listener.lookupTransform("/world", "/gvrbotE/base",  
